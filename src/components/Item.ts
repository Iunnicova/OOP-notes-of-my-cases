...
//*Объект Item предназначен для того чтобы создавать разметку карточки и с помощью методов нам будет удобно с ней в последсевии работать
	          ...
	          //** */ разбивка построчно
	//+в конструкторе мы нашли элементы разметки строка 30 и 33 которыми мы будем пользоваться в методах и создали метод 39 строка который возвращает, генерирует нам окончательную разметку карточки метод рендер принимает название заполняет textContent у title и возвращает нам itemElement
